...
class PPO:
    ...
    def update_policy(self, obs_batch, action_batch, return_batch, advantage_batch, mask, env_fn):
        policy = self.policy
        critic = self.critic
        ...
        for itr in range(n_itr):
            ...
            for epoch in range(self.epochs):
                ...
                for indices in sampler:
                    ...
                    scalars = self.update_policy(obs_batch, action_batch, return_batch, advantage_batch, mask, env_fn)
            ...
            if logger is not None:
                ...
                test = self.sample_parallel(env_fn, self.policy, self.critic, self.num_steps // 2, self.max_traj_len,
                                            deterministic=True)
...
def run_experiment(args):
    ...
    if args.previous is not None:
        ...
        if args.recurrent:
            policy = LSTM_model(obs_dim, action_dim)
            critic = LSTM_Critic(obs_dim)
        else:
            policy = line_model(obs_dim, action_dim)
            critic = LSTM_Critic(obs_dim)
