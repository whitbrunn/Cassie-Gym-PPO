...
class line_model(Net):  # 定义线性模型
    ...
    def forward(self, state, deterministic=True, anneal=1.0):
        ...
        self.action = torch.tanh(self.action)
        return self.action
...
class LSTM_Critic(Net):
    ...
    def init_hidden_state(self, batch_size=1):
        ...
        self.hidden = [torch.zeros(batch_size, l.hidden_size, device=device) for l in self.critic_layers]
        self.cells = [torch.zeros(batch_size, l.hidden_size, device=device) for l in self.critic_layers]
